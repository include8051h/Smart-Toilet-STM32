...
uint8_t status_wind = 0;  // 0=关闭, 1=开启,用于向onenet上报风扇的状态,初始状态为false
...
//开漏输出通常用于总线（如I2C）或者需要电平转换的场合。
	...
	GPIO_Init(Wind_Port, &GPIO_InitStructure);//初始化
...
void Wind_OFF(void)
{
	GPIO_ResetBits(Wind_Port, Wind_Pin);//将风扇控制引脚置低电平
	status_wind = 0;//更新状态
}
...
void Wind_ON(void)
{
	GPIO_SetBits(Wind_Port, Wind_Pin);//将风扇的控制引脚置为高电平
	status_wind = 1;//更新状态
}
...
void sendToOnenet_status_wind(void)
{
    ...
    sprintf(cmd, "AT+MQTTPUB=0,\"$sys/r7vg0Nn75e/upper_computer/thing/property/post\","
                "\"{\\\"id\\\":\\\"123\\\"\\,\\\"params\\\":{\\\"wind\\\":{\\\"value\\\":%s\\}}}\",0,0", 
                status_wind ? "true" : "false");
